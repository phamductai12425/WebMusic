        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-24 animate-fade-in-up">
            @forelse ($songs as $index => $song)
                <div class="song-card bg-gray-800/50 hover:bg-gray-800/70 rounded-2xl p-4 transition-all duration-300 cursor-pointer group relative overflow-hidden" data-index="{{ $index }}">
                    <div class="relative">
                        <img src="{{ $song->image_path ? asset('storage/' . $song->image_path) : 'https://source.unsplash.com/random/300x300/?music,album' }}" alt="{{ $song->title }}" class="w-full h-48 object-cover rounded-xl mb-4">
            ...
            <!-- Add New Song Card -->
            <div class="border-2 border-dashed border-gray-700 hover:border-purple-500 rounded-2xl p-4 transition-all duration-300 cursor-pointer flex flex-col items-center justify-center group">
                <div class="w-16 h-16 bg-gray-800 rounded-full flex items-center justify-center mb-4 group-hover:bg-purple-900/30 transition-colors duration-300">
                    <i class="fas fa-plus text-2xl text-gray-400 group-hover:text-purple-400 transition-colors duration-300"></i>
                </div>
                <h3 class="font-semibold text-lg text-gray-300 group-hover:text-purple-300 transition-colors duration-300">Thêm bài hát mới</h3>
                <p class="text-sm text-gray-500">Nhấn để tải lên</p>
            </div>
        </div>
    </div>
